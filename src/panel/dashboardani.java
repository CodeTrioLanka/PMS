 ...
 * @author ravin
 */
public class dashboardani extends javax.swing.JPanel {

    /**
     * Creates new form dashboardani
     */
    public dashboardani() {
        initComponents();
    ...
    try {
       ...
       // String sql="select date as Month,sum(totalqty) as quantity,sum(wet_weight) as wetweight,sum(sackweight)as sackweight from supplier_payment group by date order by date ASC";
         String sql="SELECT DATE_FORMAT(date, '%M') AS month, SUM(qty) AS total,SUM(w_weight) AS wet_weight,SUM(s_weight) AS Sack_Weight FROM supplier_supply GROUP BY DATE_FORMAT(date, '%M') ORDER BY MAX(date)DESC";
        ...
        while(r.next()){
            ...
            double Wet_weight=r.getDouble("wet_weight");
            double sack_weight=r.getDouble("Sack_Weight");
